...
void User_Interface::drawForce(float forceInput)
{
    ...
    int16_t force = static_cast<int16_t>(round(constrain(forceInput, -20000, 20000)));
...
void User_Interface::drawPageBar(bool cell, bool sd, bool rf, bool armed, bool ready, float battPer, bool forceUpdateAll)
{
    ...
    // Update Battery icon if the displayed icon (rounded to nearest 10) is different
...
void User_Interface::drawIntroPage()
{
    ...
    display.drawBitmap(
        ...
        logo.bmp, logo.width, logo.height, 1);
